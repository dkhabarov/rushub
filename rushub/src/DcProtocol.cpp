...
int DcProtocol::DoCmd(Parser *parser, Conn *conn) {
	...
	switch(parser->miType) {
		...
		case NMDC_TYPE_SEARCH:        DC_Search(dcparser, dcconn); break;
		case NMDC_TYPE_SR:            DC_SR(dcparser, dcconn); break;
		case NMDC_TYPE_MYNIFO:        DC_MyINFO(dcparser, dcconn); break;
		case NMDC_TYPE_SUPPORTS:      DC_Supports(dcparser, dcconn); break;
		case NMDC_TYPE_KEY:           DC_Key(dcparser, dcconn); break;
		case NMDC_TYPE_VALIDATENICK:  DC_ValidateNick(dcparser, dcconn); break;
		case NMDC_TYPE_VERSION:       DC_Version(dcparser, dcconn); break;
		case NMDC_TYPE_GETNICKLIST:   DC_GetNickList(dcparser, dcconn); break;
		case NMDC_TYPE_CHAT:          DC_Chat(dcparser, dcconn); break;
		case NMDC_TYPE_TO:            DC_To(dcparser, dcconn); break;
		case NMDC_TYPE_MYPASS:        DC_MyPass(dcparser, dcconn); break;
		case NMDC_TYPE_CONNECTTOME:   DC_ConnectToMe(dcparser, dcconn); break;
		case NMDC_TYPE_RCONNECTTOME:  DC_RevConnectToMe(dcparser, dcconn); break;
		case NMDC_TYPE_MCONNECTTOME:  DC_MultiConnectToMe(dcparser, dcconn); break;
		case NMDC_TYPE_KICK:          DC_Kick(dcparser, dcconn); break;
		case NMDC_TYPE_OPFORCEMOVE:   DC_OpForceMove(dcparser, dcconn); break;
		case NMDC_TYPE_GETINFO:       DC_GetINFO(dcparser, dcconn); break;
		case NMDC_TYPE_MCTO:          DC_MCTo(dcparser, dcconn); break;
		case NMDC_TYPE_PING:          DC_Ping(dcparser, dcconn); break;
		case NMDC_TYPE_UNKNOWN:       DC_Unknown(dcparser, dcconn); break;
		case NMDC_TYPE_QUIT:          DC_Quit(dcparser, dcconn); break;
		case NMDC_TYPE_UNPARSED:      dcparser->Parse(); return DoCmd(parser, dcconn);
...
int DcProtocol::DC_Supports(DcParser *dcparser, DcConn *dcconn) {
	...
	is >> sFeature;
	while(sFeature.size()) {
		if(sFeature      == "UserCommand") dcconn->mFeatures |= SUPPORT_FEATUER_USERCOMMAND;
		else if(sFeature == "NoGetINFO")   dcconn->mFeatures |= SUPPORT_FEATUER_NOGETINFO;
		else if(sFeature == "NoHello")     dcconn->mFeatures |= SUPPORT_FEATUER_NOHELLO;
		else if(sFeature == "UserIP2")     dcconn->mFeatures |= SUPPORT_FEATUER_USERIP2;
		else if(sFeature == "TTHSearch")   dcconn->mFeatures |= SUPPORT_FEATUER_TTHSEARCH;
		else if(sFeature == "QuickList")   dcconn->mFeatures |= SUPPORT_FEATUER_QUICKLIST;
		sFeature.clear();
		is >> sFeature;
	}
	...
	static string sMsg("$Supports UserCommand NoGetINFO NoHello UserIP2 MCTo"NMDC_SEPARATOR);
...
int DcProtocol::DC_ValidateNick(DcParser *dcparser, DcConn *dcconn) {
	...
	try {
		...
		if(mDcServer->ErrLog(0)) mDcServer->LogStream() << "Unhandled exception in DcProtocol::DC_ValidateNick" << endl;
...
int DcProtocol::DC_GetNickList(DcParser *dcparser, DcConn *dcconn) {
	...
	if(!dcconn->GetLSFlag(LOGIN_STATUS_MYINFO) && mDcServer->mDcConfig.mbNicklistOnLogin) {
		if(mDcServer->mDcConfig.mbDelayedLogin) {
			...
			if(LSFlag & LOGIN_STATUS_NICKLST) LSFlag -= LOGIN_STATUS_NICKLST;
...
int DcProtocol::DC_MyINFO(DcParser *dcparser, DcConn *dcconn) {
	...
	if(!dcconn->mDCUser) {
		...
		//} else
		{
			if(dcconn->Log(2)) dcconn->LogStream() << "Myinfo without user: " << dcparser->mCommand << endl;
			mDcServer->sendToUser(dcconn, mDcServer->mDCLang.msBadLoginSequence.c_str(), (char*)mDcServer->mDcConfig.msHubBot.c_str());
...
int DcProtocol::DC_OpForceMove(DcParser *dcparser, DcConn *dcconn) {
	...
	/** Is user exist? */
	if(!User || !User->mDCConn || !dcparser->chunkString(CHUNK_FM_DEST).size()) return -3;
...
int DcProtocol::DC_Unknown(DcParser *dcparser, DcConn *dcconn) {
	...
	if(dcparser->mCommand.compare(0, 1, "$") && mDcServer->mDcConfig.mbDisableNoDCCmd) {
		if(dcconn->Log(1)) dcconn->LogStream() << "Bad DC cmd: " << dcparser->mCommand.substr(0, 10) << " ..., close" << endl;
...
// $Lock ...|
string & DcProtocol::Append_DC_Lock(string &sStr) {
...
// $Hello sNick|
string & DcProtocol::Append_DC_Hello(string &sStr, const string &sNick) {
	static const char * cmd = "$Hello ";
	static unsigned int cmdLen = 7 + NMDC_SEPARATOR_LEN;
	sStr.reserve(sStr.size() + sNick.size() + cmdLen);
	return sStr.append(cmd, 7).append(sNick).append(NMDC_SEPARATOR, NMDC_SEPARATOR_LEN);
}

// $HubIsFull|
string & DcProtocol::Append_DC_HubIsFull(string &sStr) {
...
// $GetPass|
string & DcProtocol::Append_DC_GetPass(string &sStr) {
...
string & DcProtocol::Append_DC_ValidateDenide(string &sStr, const string &sNick) {
	static const char * cmd = "$ValidateDenide ";
	static unsigned int cmdLen = 16 + NMDC_SEPARATOR_LEN;
	sStr.reserve(sStr.size() + sNick.size() + cmdLen);
	return sStr.append(cmd, 16).append(sNick).append(NMDC_SEPARATOR, NMDC_SEPARATOR_LEN);
}

// $HubName sHubName - sTopic|
string & DcProtocol::Append_DC_HubName(string &sStr, const string &sHubName, const string &sTopic) {
	...
	if(sTopic.length()) {
		...
		return sStr.append(cmd, 9).append(sHubName).append(cmd2, 3).append(sTopic).append(NMDC_SEPARATOR, NMDC_SEPARATOR_LEN);
	} else {
		sStr.reserve(sStr.size() + sHubName.size() + cmdLen);
		return sStr.append(cmd, 9).append(sHubName).append(NMDC_SEPARATOR, NMDC_SEPARATOR_LEN);
	}
}

// $HubTopic sHubTopic|
string & DcProtocol::Append_DC_HubTopic(string &sStr, const string &sHubTopic) {
	static const char * cmd = "$HubTopic ";
	static unsigned int cmdLen = 10 + NMDC_SEPARATOR_LEN;
	sStr.reserve(sStr.size() + sHubTopic.size() + cmdLen);
	return sStr.append(cmd, 10).append(sHubTopic).append(NMDC_SEPARATOR, NMDC_SEPARATOR_LEN);
}

// <sNick> sMsg|
string & DcProtocol::Append_DC_Chat(string &sStr, const string &sNick, const string &sMsg) {
	static const char * cmd = "<";
	static const char * cmd2 = "> ";
	static unsigned int cmdLen = 3 + NMDC_SEPARATOR_LEN;
	sStr.reserve(sStr.size() + sNick.size() + sMsg.size() + cmdLen);
	return sStr.append(cmd, 1).append(sNick).append(cmd2, 2).append(sMsg).append(NMDC_SEPARATOR, NMDC_SEPARATOR_LEN);
}

// $To: sTo From: sFrom $<sNick> sMsg|
string & DcProtocol::Append_DC_PM(string &sStr, const string &sTo, const string &sFrom, const string &sNick, const string &sMsg) {
	...
	return sStr.append(cmd4, 2).append(sMsg).append(NMDC_SEPARATOR, NMDC_SEPARATOR_LEN);
}

// $To: sTo From: sFrom $<sNick> sMsg|
void DcProtocol::Append_DC_PMToAll(string &sStart, string &sEnd, const string &sFrom, const string &sNick, const string &sMsg) {
	static const char * cmd = "$To: ";
	static const char * cmd2 = " From: ";
	static const char * cmd3 = " $<";
	static const char * cmd4 = "> ";
	...
	sEnd.append(cmd4, 2).append(sMsg).append(NMDC_SEPARATOR, NMDC_SEPARATOR_LEN);
}

// $Quit sNick|
string & DcProtocol::Append_DC_Quit(string &sStr, const string &sNick) {
	static const char * cmd = "$Quit ";
	static unsigned int cmdLen = 6 + NMDC_SEPARATOR_LEN;
	sStr.reserve(sStr.size() + sNick.size() + cmdLen);
	return sStr.append(cmd, 6).append(sNick).append(NMDC_SEPARATOR, NMDC_SEPARATOR_LEN);
}

// $OpList sNick$$|
string & DcProtocol::Append_DC_OpList(string &sStr, const string &sNick) {
...
// $UserIP sNick sIP$$|
string & DcProtocol::Append_DC_UserIP(string &sStr, const string &sNick, const string &sIP) {
...
string & DcProtocol::Append_DC_ForceMove(string &sStr, const string &sAddress) {
	static const char * cmd = "$forceMove ";
	sStr.reserve(sAddress.size() + 11 + NMDC_SEPARATOR_LEN);
	return sStr.append(cmd, 11).append(sAddress).append(NMDC_SEPARATOR, NMDC_SEPARATOR_LEN);
}
...
void DcProtocol::SendMode(DcConn *dcconn, const string & sStr, int iMode, UserList & UL, bool bUseCache) {
	bool bAddSep = false;
	if(sStr.substr(sStr.size() - 1, 1) != NMDC_SEPARATOR) bAddSep = true;
